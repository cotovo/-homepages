...
export default function App() {
  ...
  // GSAP transition when active screen changes
  useEffect(() => {
    gsap.fromTo(
      '.gsap-animate-title',
  ...
  // Wheel slide navigation (desktop only)
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      // If the scroll is happening inside a scrollable container, don't trigger page transition
      ...
      if (target.closest('.no-scrollbar') || target.closest('#screen-section-trajectory') || target.closest('#screen-section-vision') || target.closest('#screen-section-access')) {
        // Only trigger if scroll reaches bottom/top, but for safety, let scroll execute
  ...
  // Touch slide support for mobile devices (with smart nested components filtering)
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    // Fully ignore swipe screen transitions if gesture happens inside core active layouts or lists
    // This allows perfect native vertical scrolling on small screens without sudden page-flipping!
    if (
      ...
      // Let standard element scrolling handle the touch
      return;
    }
  ...
  // Static backdrop particle grid
  const backgroundNodes = Array.from({ length: 30 }).map((_, i) => ({
    id: i,
  ...
  return (
    <div 
      ...
      className="relative w-full h-screen overflow-hidden bg-[#fafafa] text-[#171717] font-sans flex flex-col justify-between select-none"
    >
      {/* 1. Backdrop Grid Dots */}
      ...
      <main className="w-full flex-grow relative px-5 md:px-12 z-20 overflow-hidden flex items-center">
        <AnimatePresence mode="wait">
          {activeScreen === 0 && (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center w-full py-4 md:py-0">
                ...
                {/* Avatar frame (Beautiful scaling on mobile targets) */}
                <div className="col-span-1 md:col-span-5 flex justify-center md:justify-end order-1 md:order-2">
                  <div className="gsap-animate-card relative w-44 h-44 xs:w-52 xs:h-52 md:w-68 md:h-68 border border-neutral-200/40 rounded-full flex items-center justify-center bg-white/40 shadow-xs">
                    ...
                    {/* Image frame */}
                    <div className="relative w-32 h-32 xs:w-38 xs:h-38 md:w-46 md:h-46 rounded-full overflow-hidden border-2 border-white shadow-md transition-transform duration-550 hover:scale-[1.04] group cursor-pointer">
                      <img 
                      ...
                      <div className="absolute inset-0 bg-neutral-950/15 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <span className="font-mono text-[8px] text-white bg-black/80 px-2 py-0.5 rounded-md">
                          QQ: 1722288011
                        </span>
                      </div>
                    </div>
          ...
          {activeScreen === 1 && (
            /* ================= PAGE 2: TRAJECTORY & HYBRID WORLD ================= */
            <motion.section
            ...
            >
              ...
              <div className="md:col-span-5 space-y-4 text-left py-2">
                ...
                <div className="flex gap-2 items-center flex-wrap pt-2 shrink-0">
                  <button
                    ...
                    onMouseEnter={() => setHoveredNode('shiyan')}
                  ...
                  <button
                    ...
                    onMouseEnter={() => setHoveredNode('wuhan')}
                    onMouseLeave={() => setHoveredNode(null)}
                  >
                    武汉 WUHAN
                  </button>
              ...
              {/* Right Hub Component (Interactive Map / Spectrum Tab) */}
          ...
          {activeScreen === 2 && (
            /* ================= PAGE 3: ACCESS POINT / SINCERE LINKS ================= */
            <motion.section
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center w-full py-2">
                ...
                {/* Right side contact links with beautiful authentic vector branding */}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3.5 text-left">
                            ...
                            <div className="space-y-0.5">
                              ...
                              <span className="text-sm font-bold tracking-wide text-neutral-900 block">
                                {link.value}
                              </span>
                              <p className="text-[10px] text-neutral-500 leading-normal font-sans block pt-0.5">
                                {link.sub}
                              </p>
                            </div>
                          </div>
      ...
      <footer className="w-full px-6 md:px-12 py-6 flex items-center justify-between z-40 shrink-0 pointer-events-none">
        ...
        {/* Tactical dot controllers & click handlers */}
        <div className="flex items-center gap-3 select-none pointer-events-auto">
            ...
            {[0, 1, 2].map((idx) => (
              <button
                ...
                className={`w-1.5 h-1.5 rounded-full transition-all duration-500 cursor-pointer ${
                  activeScreen === idx ? 'bg-neutral-900 w-5' : 'bg-neutral-200 hover:bg-neutral-400'
                }`}
              />
            ))}
          </div>
          ...
          <div className="flex gap-1 ml-3 border-l border-neutral-200 pl-3">
            <button
              ...
              className={`p-1 transition-colors cursor-pointer ${activeScreen === 0 ? 'text-neutral-200 cursor-not-allowed' : 'text-neutral-400 hover:text-neutral-950'}`}
            >
            ...
            <button
              ...
              className={`p-1 transition-colors cursor-pointer ${activeScreen === 2 ? 'text-neutral-200 cursor-not-allowed' : 'text-neutral-400 hover:text-neutral-950'}`}
            >
